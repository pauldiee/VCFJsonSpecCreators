...
$border  = '═' * 58
$blank   = '║' + (' ' * 58) + '║'
$title   = 'VCF Cluster Spec Creator'
...
if ($MockMode) {
    ...
    Write-Host '  Stub data is used for domains, hosts, pools, and validation' -ForegroundColor DarkYellow
    Write-Host $mockBanner -ForegroundColor DarkYellow
}
#endregion ───────────────────────────────────────────────────────────────────

#region ── Step 1: SDDC Manager connection ────────────────────────────────────
Write-Header 'Step 1 of 7  |  SDDC Manager Connection'
...
Write-Header 'Step 2 of 7  |  Target Workload Domain'

if ($MockMode) {
    Write-Mock 'Using mock domain list.'
    $domainList = $MockDomains
} else {
    Write-Step 'Querying workload domains from SDDC Manager ...'
    try {
    ...
    if (-not $domainList -or $domainList.Count -eq 0) {
        Write-Fail 'No workload domains found in SDDC Manager.'
        exit 1
    }
}
...
if ($domainIdx -lt 0 -or $domainIdx -ge $domainList.Count) {
    Write-Fail 'Invalid domain selection.'
    exit 1
}
$selectedDomain = $domainList[$domainIdx]
Write-OK "Target domain: $($selectedDomain.name)  (ID: $($selectedDomain.id))"
#endregion ───────────────────────────────────────────────────────────────────

#region ── Step 3: Select hosts ───────────────────────────────────────────────
Write-Header 'Step 3 of 7  |  Host Selection'
...
if ($selectedHosts.Count -lt 2) {
    Write-Warn 'At least 2 hosts are recommended for a cluster.'
}
...
Write-Header 'Step 4 of 7  |  Storage Type Detection'
...
Write-Header 'Step 5 of 7  |  Cluster and vSAN Configuration'
...
Write-OK "vSAN datastore name: $vsanDatastoreName"
...
if ($FailuresToTolerate -and $FailuresToTolerate.Trim() -ne '') {
    if ($FailuresToTolerate.Trim() -in @('1', '2')) {
        ...
        Write-OK "Failures to tolerate (pre-filled): $fttResolved"
    } else {
        Write-Warn "Pre-filled FailuresToTolerate '$FailuresToTolerate' must be 1 or 2. Please select."
    }
}
...
if ($null -eq $fttResolved) {
    ...
    while ($fttChoice -notin @('1', '2')) {
        ...
        if ($fttChoice -notin @('1', '2')) { Write-Warn 'Please enter 1 or 2.' }
    }
    $fttResolved = [int]$fttChoice
}
Write-OK "Failures to tolerate: $fttResolved"
#endregion ───────────────────────────────────────────────────────────────────

#region ── Step 6: VDS and network configuration ─────────────────────────────
Write-Header 'Step 6 of 7  |  VDS and Network Configuration'
...
if ($MockMode) {
    ...
    Write-Host ''
    Write-OK 'Validation PASSED (mock). Cluster JSON is ready for review.'
} else {
    Write-Step 'Submitting validation request to /v1/clusters/validations ...'
    $validationResp = $null
    try {
    ...
    if ($validationResp) {
        ...
        if ($finalStatus -eq 'COMPLETED') {
            if ($poll.resultStatus -eq 'SUCCEEDED') {
                Write-OK 'Validation PASSED. Cluster JSON is ready for deployment.'
            } else {
