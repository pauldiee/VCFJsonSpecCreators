...
- Full input validation: FQDNs, IP addresses, VLAN IDs, CIDRs, and passwords
...
| 5 | VDS name, MTU, uplinks, VLAN IDs, TEP IP pool |
...
| 6 | VDS name, MTU, uplinks, VLAN IDs, TEP IP pool, network pool |
...
| 5 | VDS name, uplinks, fault domain names |
...
- **Input validation** — FQDNs, IP addresses, VLAN IDs, CIDRs, and passwords are validated before the JSON is built
- **deployWithoutLicenseKeys** — all payloads include `deployWithoutLicenseKeys: true` by default (VCF 9 consumption-based licensing)
